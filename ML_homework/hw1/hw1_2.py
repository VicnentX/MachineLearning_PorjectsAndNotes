...
# add one col to the left of raw_data
...
for n in range(20, 101, 20):
    ...
    for iter in range(1000):
        ...
        while len(test_data_0) < n or len(test_data_0) < n:
            ...
            if raw_data_plus_bias[index, col_cnt - 1] == 0 \
                    and len(test_data_0) < n and index not in test_data_0:
                test_data_0.add(index)
                x[point, :] = raw_data_plus_bias[index, :col_cnt - 1]
                t[point] = 0
            if raw_data_plus_bias[index, col_cnt - 1] == 1 \
                    and len(test_data_1) < n and index not in test_data_1:
